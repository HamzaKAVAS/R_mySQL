...
SELECT kelime FROM kelimeler WHERE kelime LIKE '_o%' AND harf_sayisi=3;
SELECT kelime FROM kelimeler WHERE kelime LIKE '_o_';
SELECT kelime FROM kelimeler WHERE REGEXP_LIKE (kelime,'^.o.$');

-- ilk harfi h olup kelime uzunluğu 3'den fazla olan kelimeleri listeleyin
SELECT kelime FROM kelimeler WHERE kelime LIKE 'h%' AND harf_sayisi>3;